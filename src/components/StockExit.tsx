...
import { Minus, Package, CheckCircle, AlertTriangle } from 'lucide-react';
import { Product, StockMovement } from '@/pages/Index';
import { toast } from '@/hooks/use-toast';

interface StockExitProps {
  products: Product[];
...
export const StockExit = ({ products, onAddMovement }: StockExitProps) => {
  ...
  const getStockWarnings = () => {
    ...
    if (formData.quantity > selectedProduct.currentStock) {
      warnings.push({
        type: 'error' as const,
        message: `Quantidade indisponível! Estoque atual: ${selectedProduct.currentStock} ${selectedProduct.unit}`,
      });
    } else {
      ...
      if (newStock <= selectedProduct.minStock) {
        warnings.push({
          ...
          message: `Atenção! Após esta saída, o estoque ficará abaixo do mínimo (${selectedProduct.minStock} ${selectedProduct.unit})`,
        });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${formData.quantity} ${selectedProduct.unit} de ${selectedProduct.name} removido do estoque.`,
      });
      ...
      <Card className="dark:bg-gray-800 dark:border-gray-700">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {selectedProduct && (
                <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  ...
                  <div className="flex items-center justify-between text-sm mt-1">
                    ...
                    <span className="dark:text-gray-200">{selectedProduct.minStock} {selectedProduct.unit}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm mt-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  required
                />
                {selectedProduct && formData.quantity > 0 && !hasError && (
            ...
            {warnings && warnings.length > 0 && (
              <div className="space-y-2">
                {warnings.map((warning, index) => (
                  <Alert key={index} variant={warning.type === 'error' ? 'destructive' : 'default'} className="dark:border-gray-600">
                    <AlertTriangle className="h-4 w-4" />
                    <AlertDescription className="dark:text-gray-300">{warning.message}</AlertDescription>
                  </Alert>
                ))}
              </div>
            )}
            ...
            <div>
              <Label htmlFor="reason" className="dark:text-gray-200">Motivo da Saída</Label>
